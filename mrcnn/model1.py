...
def conv_block(input_tensor, kernel_size, filters, stage, block,
    ...
    x = KL.Conv2D(nb_filter1, (1, 1), strides=strides,  # 整个conv_block只下采样了一次
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        # 长宽没有指定，channel指定了
        # 从config中看，这个图像的长宽应该是1024*1024，先用resize再用zero padding填充，变成这个尺寸。
        input_image_meta = KL.Input(shape=[config.IMAGE_META_SIZE],
                                    name="input_image_meta")
        # image_meta存了一张图片的所有信息，比如image_shape（经过resize和padding的），有效区域windows(实际图片所在的位置)。
        # 数据集包含的所有的图像类等
        if mode == "training":
            # RPN GT
        ...
        if mode == "training":
            ...
            # 看起来有点多余了，但是keras要求这么做
            anchors = np.broadcast_to(anchors, (config.BATCH_SIZE,) + anchors.shape)
            # A hack to get around Keras's bad support for constants
            anchors = KL.Lambda(lambda x: tf.Variable(anchors), name="anchors")(input_image)
            # 这里好像无论输入什么input_image，输出都是tf.Variable(anchors)，和input_image没有关系
            # 可能这也是keras要求的吧，虽然是常数，也需要一个输入
        else:
        ...
        if mode == "training":
            ...
            # 利用classifier生成的box和class确定要生成哪个累的mask，每一个box都对应一个类，即上面的class_id。

            # Create masks for detections
            detection_boxes = KL.Lambda(lambda x: x[..., :4])(detections)  # [batch, num_detections, (y1, x1, y2, x2)]
    ...
    def get_anchors(self, image_shape):  # image_shape = [1024*1024]
